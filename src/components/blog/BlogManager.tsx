...
type BlogForm = Omit<BlogPost, "id" | "createdAt"> & { imageFile?: File | null };

const emptyForm: BlogForm = { title: "", excerpt: "", category: "Growth", author: "Sellers Login", imageUrl: "", imagePreview: "", imageFile: null };
...
export function BlogManager() {
  ...
  const openEdit = (post: BlogPost) => {
    ...
    setForm({
      ...
      author: post.author,
      imageUrl: post.imageUrl,
      imagePreview: post.imagePreview || post.imageUrl,
      imageFile: null,
    });
    setIsEditorOpen(true);
  };
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (savedPost && typeof savedPost !== "boolean") {
      const resultPost = form.imagePreview ? { ...savedPost, imagePreview: form.imagePreview } : savedPost;
      const nextPosts = editingId ? posts.map((item) => (item.id === editingId ? resultPost : item)) : [resultPost, ...posts];
      setPosts(nextPosts);
  ...
  return (
    <>
      ...
      <main className="min-h-screen bg-gray-50 px-4 pb-20 pt-28 sm:px-6 lg:px-8">
        <section className="mx-auto max-w-7xl">
          ...
          <div className="mt-8 grid gap-6 md:grid-cols-2 xl:grid-cols-3">
            {posts.map((post) => (
              <article key={post.id} className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-lg">
                <Link href={`/resources/blog/${encodeURIComponent(post.id)}`} className="block focus:outline-none focus-visible:ring-2 focus-visible:ring-purple-500 focus-visible:ring-offset-2">
                  <div className="h-44 bg-linear-to-br from-purple-100 via-white to-indigo-100">{(post.imagePreview || post.imageUrl) && <img src={post.imagePreview || post.imageUrl} alt="" className="h-full w-full object-cover" />}</div>
                  <div className="p-6">
                    ...
                    <h2 className="mt-4 text-xl font-bold leading-snug text-gray-950">{post.title}</h2>
                    <p className="mt-3 line-clamp-3 text-sm leading-relaxed text-gray-600">{post.excerpt}</p>
                    <p className="mt-5 text-sm font-medium text-gray-700">By {post.author}</p>
                  </div>
                </Link>
                <div className="border-t border-gray-100 p-4">
                  <div className="flex flex-wrap gap-3">
                    <button onClick={() => openEdit(post)} className="text-sm font-bold text-purple-700 hover:text-purple-900">Edit</button>
                    <button onClick={() => removePost(post)} className="text-sm font-bold text-red-600 hover:text-red-800">Delete</button>
                  </div>
                </div>
              </article>
            ))}
      ...
      {isEditorOpen && <div className="fixed inset-0 z-[60] flex items-center justify-center bg-gray-950/60 p-4" role="dialog" aria-modal="true" aria-label="Blog post editor"><form onSubmit={submit} className="max-h-[90vh] w-full max-w-xl overflow-y-auto rounded-2xl bg-white p-6 shadow-2xl"><div className="flex items-center justify-between"><h2 className="text-2xl font-bold text-gray-950">{editingId ? "Edit post" : "New post"}</h2><button type="button" onClick={() => setIsEditorOpen(false)} className="text-gray-500 hover:text-gray-950" aria-label="Close editor">✕</button></div><div className="mt-6 grid gap-4"><label className="text-sm font-semibold text-gray-700">Title<input required value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} className="mt-1.5 w-full rounded-lg border border-gray-300 px-3 py-2 text-gray-950" /></label><label className="text-sm font-semibold text-gray-700">Excerpt<textarea required rows={4} value={form.excerpt} onChange={(e) => setForm({ ...form, excerpt: e.target.value })} className="mt-1.5 w-full rounded-lg border border-gray-300 px-3 py-2 text-gray-950" /></label><div className="grid gap-4 sm:grid-cols-2"><label className="text-sm font-semibold text-gray-700">Category<input required value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })} className="mt-1.5 w-full rounded-lg border border-gray-300 px-3 py-2 text-gray-950" /></label><label className="text-sm font-semibold text-gray-700">Author<input required value={form.author} onChange={(e) => setForm({ ...form, author: e.target.value })} className="mt-1.5 w-full rounded-lg border border-gray-300 px-3 py-2 text-gray-950" /></label></div><label className="text-sm font-semibold text-gray-700">
                    Image file <span className="font-normal text-gray-400">(local only)</span>
                    <input
                      type="file"
                      accept="image/*"
                      onChange={(e) => {
                      ...
                      className="mt-1.5 w-full rounded-lg border border-gray-300 px-3 py-2 text-gray-950 file:mr-4 file:rounded-full file:border-0 file:bg-purple-200 file:px-3 file:py-2 file:text-sm file:font-semibold file:text-gray-950"
                    />
                  </label>
                  {form.imagePreview && (
                    <div className="overflow-hidden rounded-xl border border-gray-200">
                      <img src={form.imagePreview} alt="Selected preview" className="h-40 w-full object-cover" />
                    </div>
                  )}
